...
Updated for 23 September 2026 with Node.js v24. Tests run the production core, storage, UI templates, and service-worker source.
...
## Version 1.9.0: verified backups and everyday tracking

191 automated tests pass. New checks exercise authenticated encrypted backup roundtrips, Unicode passwords, wrong passwords and tampering, bounded imports, legacy plain backups, exact-file verification, cancelled sharing, restore confirmation and undo, non-overlapping overview totals, retained-history setup, quick preferences, expense repeats, recurring month-end/leap anchors and review-only entry generation, concurrent duplicate rejection, rollback, stale restore reviews, and version/waiting status. A synthetic fixture generated by the previous 1.8.0 core is reopened unchanged and roundtripped through the new encrypted format.

No real iPhone, Safari or visual-browser test is claimed. No independent security review has been performed. See [physical acceptance matrix](IPHONE-ACCEPTANCE.md), [security review handoff](SECURITY-REVIEW.md), and [release behaviour](RELEASE-1.9.md).


## Version 1.10.0: family workflows
