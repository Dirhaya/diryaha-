...
function activeAccounts(state){return state.accounts.filter(a=>!a.archived)}
...
function resetMoney(state){const fresh=blank();fresh.rev=state.rev+1;fresh.accounts=state.accounts.map(a=>{const account={...clone(a),opening:0};delete account.lastCheck;delete account.holdings;return account});fresh.cards=clone(state.cards);fresh.goals=state.goals.map(g=>({...clone(g),allocations:[]}));if(state.settings.reminderTimes)fresh.settings.reminderTimes=clone(state.settings.reminderTimes);for(const key of ['name','theme','hideAmounts','needsName','gharsWelcomed','quickCategories','defaultPayment','language','textSize','setupDone'])fresh.settings[key]=state.settings[key];validate(fresh);return fresh}
...
function assertReserves(state){const b=balances(state),r=reserves(state);for(const a of state.accounts){if(a.archived&&(b[a.id]!==0||r[a.id]||a.holdings?.length))fail('Closed accounts must keep a zero balance and no holdings or goal savings. Reopen the account before changing its history.');if(b[a.id]<0)fail(`“${a.name}” would have a negative balance. Check the opening balance or record missing income first.`);if(holdingsValue(a)>b[a.id])fail('This account contains investments. Sell or update the holdings before moving their value out.');if(r[a.id]>b[a.id])fail(`Some money in “${a.name}” is reserved for goals. Release enough goal money first.`)}}
...
const UNDO_COLLECTIONS=['accounts','cards','transactions','goals','recurring'];
const UNDO_SETTINGS=['defaultPayment','trackingFrom','trackingStartedAt'];
function isMoneyAction(type){return /^(account|card|transaction|goal|holding|recurring)\./.test(type)}
function changeLabel(action){const labels={'account.archive':'Account removed from active list','account.reopen':'Account reopened','transaction.add':'Entry added','transaction.edit':'Entry edited','transaction.delete':'Entry deleted','transaction.split':'Entry split','transaction.explain':'Unexplained amount categorised','account.check':'Balance checked','account.reconcile':'Balance corrected','recurring.record':'Recurring entry recorded','recurring.match':'Existing entry linked','recurring.skip':'Recurring occurrence skipped','holding.add':'Holding added','holding.edit':'Holding valuation changed','holding.sell':'Holding sold','goal.allocate':'Money reserved for a goal','goal.release':'Goal money released','goal.move':'Goal money moved'};return labels[action.type]||({'add':'Added','save':'Saved','edit':'Edited','delete':'Deleted','remove':'Removed'}[action.type.split('.')[1]]||'Changed')+' '+action.type.split('.')[0]}
function recordChange(before,after,action){
 const changes=[];for(const collection of UNDO_COLLECTIONS){const old=before[collection]||[],next=after[collection]||[],oldMap=new Map(old.map(x=>[x.id,x])),nextMap=new Map(next.map(x=>[x.id,x]));for(const id of new Set([...oldMap.keys(),...nextMap.keys()])){const a=oldMap.get(id)||null,b=nextMap.get(id)||null;if(JSON.stringify(a)!==JSON.stringify(b))changes.push({collection,id,before:a,after:b,index:Math.max(0,old.findIndex(x=>x.id===id))})}}
 const settings=UNDO_SETTINGS.filter(key=>JSON.stringify(before.settings[key]??null)!==JSON.stringify(after.settings[key]??null)).map(key=>({key,before:before.settings[key]??null,after:after.settings[key]??null}));
 if(!changes.length&&!settings.length)return;
 const entry={id:uid(),label:changeLabel(action),at:Date.now(),changes,settings};after.undoHistory=[...(before.undoHistory||[]),clone(entry)].slice(-20);while(JSON.stringify(after.undoHistory).length>2000000)after.undoHistory.shift();
}
function duplicateToken(t){return [t.id,t.type,t.amount,t.account,t.date,t.category].join('|')}
function possibleDuplicates(state,action){
 if(!['income','expense'].includes(action.kind)||!Number.isSafeInteger(action.amount)||!validDate(action.date))return [];
 const targetDate=Date.parse(action.date+'T00:00:00Z');return state.transactions.filter(t=>{
  if(t.id===action.id||t.type!==action.kind||t.account!==action.account)return false;
  const days=Math.abs(Date.parse(t.date+'T00:00:00Z')-targetDate)/86400000,diff=Math.abs(t.amount-action.amount);
  return (t.amount===action.amount&&days<=3)||(t.category===action.category&&days<=1&&diff<=Math.max(100,Math.floor(action.amount*.02)))||(action.category==='Salary'&&t.category==='Salary'&&t.date.slice(0,7)===action.date.slice(0,7)&&diff<=Math.max(100,Math.floor(action.amount*.02)));
 }).sort((a,b)=>Math.abs(a.amount-action.amount)-Math.abs(b.amount-action.amount)||Math.abs(new Date(a.date)-targetDate)-Math.abs(new Date(b.date)-targetDate));
}
function guardDuplicates(state,action){const matches=possibleDuplicates(state,action),accepted=action.acknowledgedDuplicates||[];if(matches.some(t=>!accepted.includes(duplicateToken(t)))){const error=new Error('Possible duplicate. Review the existing entries before saving.');error.code='POSSIBLE_DUPLICATE';error.matches=matches.slice(0,5).map(clone);error.tokens=matches.map(duplicateToken);throw error}}
function recurringMatches(state,id){const r=(state.recurring||[]).find(r=>r.id===id);if(!r)return [];return state.transactions.filter(t=>t.type===r.kind&&t.account===r.account&&!t.recurringId&&!t.occurrence).sort((a,b)=>Math.abs(new Date(a.date)-new Date(r.nextDate))-Math.abs(new Date(b.date)-new Date(r.nextDate))||Math.abs(a.amount-r.amount)-Math.abs(b.amount-r.amount))}
function backupReminder(state,now=Date.now()){
 const settings=state.settings;if(settings.backupReminders===false||settings.backupSnoozeUntil>now||!state.accounts.length)return null;
 const baseline=settings.backupBaseline,changes=Math.max(0,(state.changeCounter||0)-(baseline?.counter||0));
 if(!baseline)return {reason:'first',changes};
 if(changes>=(settings.backupReminderChanges||20))return {reason:'changes',changes};
 if(now-baseline.at>=(settings.backupReminderDays||7)*86400000)return {reason:'time',changes};return null;
}
function accountRemoval(state,id){
 const account=state.accounts.find(a=>a.id===id);if(!account)fail('Choose an existing account.');
 const entries=state.transactions.filter(t=>t.account===id||t.to===id),goals=state.goals.filter(g=>g.allocations.some(a=>a.account===id)),cards=state.cards.filter(c=>c.account===id),favourites=(state.favourites||[]).filter(f=>f.account===id),recurring=(state.recurring||[]).filter(r=>r.account===id),holdings=account.holdings||[];
 return {entries,goals,cards,favourites,recurring,holdings,canDelete:!entries.length&&!goals.length&&!favourites.length&&!recurring.length&&!holdings.length};
}
function apply(state,action,{recordUndo=true}={}){
 ...
 const requested=[action.account,action.source,action.to,...(['account.edit','account.check','account.reconcile'].includes(action.type)?[action.id]:[])];
 if(requested.some(id=>s.accounts.some(a=>a.id===id&&a.archived)))fail('This account is closed in Ghars. Reopen it before recording or changing money.');
 switch(action.type){
  case 'account.archive':{
   const a=getAccount(action.id);if(a.archived)fail('This account is already closed.');
   if(balances(s)[a.id]!==0)fail('Move or correct the remaining balance before closing this account.');
   if(reserves(s)[a.id]||a.holdings?.length)fail('Move goal savings and keep investment holdings in an active account before closing.');
   a.archived=true;for(const r of s.recurring||[])if(r.account===a.id)r.paused=true;
   if(s.settings.defaultPayment===a.id||s.cards.some(c=>c.account===a.id&&s.settings.defaultPayment==='card:'+c.id))s.settings.defaultPayment='';break;
  }
  case 'account.reopen':{const a=getAccount(action.id);delete a.archived;break;}
  ...
  case 'account.delete':{
   const linked=accountRemoval(s,action.id);
   if(linked.entries.length)fail('This account has recorded entries, including transfers or balance corrections. Keep it to preserve your history.');
   if(linked.goals.length)fail('This account holds goal savings. Move or release them before deleting it.');
   if(linked.holdings.length)fail('Remove the holding details before deleting this account.');
   if(linked.favourites.length)fail('Remove favourites using this account first.');
   if(linked.recurring.length)fail('Remove recurring entries using this account first.');
   if(linked.cards.length&&action.removeUnusedCards!==true)fail('This account has linked debit cards. Confirm removal of its unused cards too.');
   if(action.expectedCards&&JSON.stringify([...action.expectedCards].sort())!==JSON.stringify(linked.cards.map(c=>c.id).sort()))fail('Linked cards changed. Reopen Delete unused account and review them again.');
   if(action.expectedOpening!==undefined&&action.expectedOpening!==getAccount(action.id).opening)fail('The account balance changed. Reopen Delete unused account and review it again.');
   const cardIds=new Set(linked.cards.map(c=>c.id));
   if(s.settings.defaultPayment===action.id||cardIds.has((s.settings.defaultPayment||'').replace(/^card:/,'')))s.settings.defaultPayment='';
   s.cards=s.cards.filter(c=>!cardIds.has(c.id));s.accounts=s.accounts.filter(a=>a.id!==action.id);break;
  }
  case 'card.add':case 'card.edit':{
   ...
   const card=action.type==='card.edit'?s.cards.find(c=>c.id===action.id):{id:uid(),created:Date.now()};if(!card)fail('Card not found.');const name=cleanName(action.name,'Card name');uniqueName(s.cards,name,card.id,'card');const last4=String(action.last4||'');if(last4&&!/^\d{4}$/.test(last4))fail('Enter only the last four digits, or leave them blank.');if(card.account&&card.account!==a.id&&(s.transactions.some(t=>t.card===card.id)||(s.recurring||[]).some(r=>r.card===card.id)||(s.favourites||[]).some(f=>f.card===card.id)))fail('This card has spending history. Add a new card to link a different bank.');Object.assign(card,{name,account:a.id,last4,color:COLORS.includes(action.color)?action.color:'slate'});if(action.type==='card.add'){if(s.cards.length>=100)fail('You can keep up to 100 cards.');s.cards.push(card)}break;
  }
  case 'card.delete':{if((s.favourites||[]).some(f=>f.card===action.id))fail('Remove favourites using this card first.');if((s.recurring||[]).some(r=>r.card===action.id))fail('Remove recurring entries using this card first.');if(s.settings.defaultPayment==='card:'+action.id)s.settings.defaultPayment='';if(!s.cards.some(c=>c.id===action.id))fail('Card not found.');if(s.transactions.some(t=>t.card===action.id))fail('This card has recorded spending. Keep it to preserve your history.');s.cards=s.cards.filter(c=>c.id!==action.id);break}
  case 'transaction.add':case 'transaction.edit':{
   ...
   if(action.checkDuplicates)guardDuplicates(s,action);
  ...
  case 'recurring.record':case 'recurring.skip':case 'recurring.match':{
   ...
   if(action.type==='recurring.record'){
    s=apply(s,{type:'transaction.add',kind:r.kind,amount:action.amount,account:action.account,card:action.card,category:r.category,date:action.date,note:action.note||r.name,checkDuplicates:action.checkDuplicates,acknowledgedDuplicates:action.acknowledgedDuplicates},{recordUndo:false});
    const t=s.transactions.at(-1);t.recurringId=r.id;t.occurrence=action.dueDate;
   }
   if(action.type==='recurring.match'){const t=s.transactions.find(t=>t.id===action.transaction);if(!t||t.type!==r.kind||t.account!==r.account)fail('Choose an entry of the same type in this account.');if(t.recurringId||t.occurrence)fail('This entry is already linked to a recurring occurrence.');t.recurringId=r.id;t.occurrence=action.dueDate;}
  ...
  case 'undo':{
   const item=s.undoHistory?.at(-1);if(!item||item.id!==action.id)fail('The latest change has changed. Reopen Undo recent changes.');
   for(const change of item.changes){const current=(s[change.collection]||[]).find(x=>x.id===change.id)||null;if(JSON.stringify(current)!==JSON.stringify(change.after))fail('Related records have changed. This action cannot be undone safely.');}
   for(const change of item.changes){s[change.collection]=s[change.collection]||[];const index=s[change.collection].findIndex(x=>x.id===change.id);if(change.before===null){if(index>=0)s[change.collection].splice(index,1)}else if(index>=0)s[change.collection][index]=clone(change.before);else s[change.collection].splice(Math.min(change.index,s[change.collection].length),0,clone(change.before));}
   for(const change of item.settings||[]){if(JSON.stringify(s.settings[change.key]??null)!==JSON.stringify(change.after))fail('Related preferences have changed. This action cannot be undone safely.');if(change.before===null)delete s.settings[change.key];else s.settings[change.key]=clone(change.before)}
   s.undoHistory.pop();assertReserves(s);break;
  }
  case 'favourite.save':{
   s.favourites=s.favourites||[];const old=s.favourites.find(f=>f.id===action.id);if(action.id&&!old)fail('This favourite no longer exists.');
   const favourite={id:old?.id||uid(),name:cleanName(action.name,'Favourite name',45),amount:cents(action.amount,{zero:false}),account:action.account,card:action.card||null,category:action.category,note:String(action.note||'').trim().slice(0,180)};uniqueName(s.favourites,favourite.name,favourite.id,'favourite');
   if(old)s.favourites[s.favourites.indexOf(old)]=favourite;else s.favourites.push(favourite);break;
  }
  case 'favourite.delete':{if(!(s.favourites||[]).some(f=>f.id===action.id))fail('Favourite not found.');s.favourites=s.favourites.filter(f=>f.id!==action.id);break}
  case 'backup.snooze':{s.settings.backupSnoozeUntil=Date.now()+86400000;break}
  case 'backup.exported':{s.settings.lastExport=clone(action.record);break}
  case 'backup.verified':{s.settings.lastVerifiedBackup=clone(action.record);if(action.current&&action.expectedRev===state.rev)s.settings.backupBaseline={at:Date.now(),counter:state.changeCounter||0};break}
  case 'settings':{for(const key of ['quickCategories','defaultPayment','language','textSize','setupStep','setupDone','backupReminders','backupReminderDays','backupReminderChanges'])if(action[key]!==undefined)s.settings[key]=clone(action[key]);if(action.gharsWelcomed!==undefined)s.settings.gharsWelcomed=!!action.gharsWelcomed;if(action.reminderTimes!==undefined){if(!Array.isArray(action.reminderTimes)||action.reminderTimes.length>24||action.reminderTimes.some(t=>typeof t!=='string'||!/^([01]\d|2[0-3]):[0-5]\d$/.test(t)))fail('Choose up to 24 valid reminder times.');s.settings.reminderTimes=[...new Set(action.reminderTimes)].sort();}if(action.name!==undefined){s.settings.name=cleanName(action.name,'Your name',35);s.settings.needsName=false;}if(action.theme!==undefined){if(!['beige','grey','night'].includes(action.theme))fail('Choose beige, grey or night.');s.settings.theme=action.theme}if(action.hideAmounts!==undefined)s.settings.hideAmounts=!!action.hideAmounts;if(action.monthlyBudget!==undefined)s.settings.monthlyBudget=cents(action.monthlyBudget);if(action.lastBackup!==undefined)s.settings.lastBackup=action.lastBackup;break}
  default:fail('That action is not supported.');
 }
 if(action.type==='tracking.start')s.undoHistory=[];else if(recordUndo&&isMoneyAction(action.type))recordChange(state,s,action);
 if(isMoneyAction(action.type)||['tracking.start','undo','favourite.save','favourite.delete'].includes(action.type))s.changeCounter=(state.changeCounter||0)+1;
...
function validate(d){
 ...
 for(const a of d.accounts){if(a.archived!==undefined&&typeof a.archived!=='boolean')fail('Invalid closed account status.');cleanName(a.name);if(names.has(a.name.toLowerCase()))fail('Duplicate account name.');names.add(a.name.toLowerCase());if(!ACCOUNT_TYPES.includes(a.type)||!COLORS.includes(a.color))fail('Invalid account.');cents(a.opening);if(a.holdings!==undefined){if(!Array.isArray(a.holdings)||a.holdings.length>500||a.type!=='investment')fail('Invalid portfolio holdings.');unique(a.holdings);for(const h of a.holdings){cleanName(h.name,'Company');shareQuantity(h.quantity);cents(h.cost);cents(h.value);if(!validDate(h.updated)||h.updated>localDate())fail('Invalid holding valuation date.')}}if(a.lastCheck){if(!validDate(a.lastCheck.date)||!Number.isFinite(a.lastCheck.at))fail('Invalid balance check.');cents(a.lastCheck.actual)}}
 ...
 if(d.recurring!==undefined){
  ...
  for(const r of d.recurring){if(d.accounts.some(a=>a.id===r.account&&a.archived)&&!r.paused)fail('Recurring entries for closed accounts must stay paused.');cleanName(r.name,'Recurring name');if(!['income','expense'].includes(r.kind)||!validCategory(r.kind,r.category)||!accounts.has(r.account)||!FREQUENCIES.includes(r.frequency)||!validDate(r.nextDate)||typeof r.paused!=='boolean'||!Number.isInteger(r.anchorDay)||r.anchorDay<1||r.anchorDay>31||!Number.isInteger(r.anchorMonth)||r.anchorMonth<1||r.anchorMonth>12)fail('Invalid recurring entry.');cents(r.amount,{zero:false});if(r.card&&!d.cards.some(c=>c.id===r.card&&c.account===r.account&&r.kind==='expense'))fail('Invalid recurring payment card.');if(!Array.isArray(r.handled)||r.handled.length>50000||new Set(r.handled).size!==r.handled.length||r.handled.some(x=>!validDate(x))||r.handled.includes(r.nextDate))fail('Invalid recurring review history.');}
 }
 if(d.changeCounter!==undefined&&(!Number.isSafeInteger(d.changeCounter)||d.changeCounter<0))fail('Invalid change counter.');
 if(d.favourites!==undefined){if(!Array.isArray(d.favourites)||d.favourites.length>30)fail('Keep up to 30 expense favourites.');unique(d.favourites);const names=new Set();for(const f of d.favourites){cleanName(f.name,'Favourite name',45);if(names.has(f.name.toLowerCase()))fail('Duplicate favourite name.');names.add(f.name.toLowerCase());cents(f.amount,{zero:false});if(!accounts.has(f.account)||!validCategory('expense',f.category)||f.category==='Unexplained'||typeof f.note!=='string'||f.note.length>180||f.card&&!d.cards.some(c=>c.id===f.card&&c.account===f.account))fail('Invalid expense favourite.');}}
 if(d.undoHistory!==undefined){
  if(!Array.isArray(d.undoHistory)||d.undoHistory.length>20||JSON.stringify(d.undoHistory).length>2000000)fail('Invalid undo history.');unique(d.undoHistory);
  for(const item of d.undoHistory){cleanName(item.label,'Undo label',100);if(!Number.isFinite(item.at)||!Array.isArray(item.changes)||item.changes.length>2000||!Array.isArray(item.settings)||item.settings.length>UNDO_SETTINGS.length)fail('Invalid undo change.');const ids=new Set();for(const c of item.changes){if(!UNDO_COLLECTIONS.includes(c.collection)||!validId(c.id)||!Number.isInteger(c.index)||c.index<0||ids.has(c.collection+':'+c.id))fail('Invalid undo record.');ids.add(c.collection+':'+c.id);for(const value of [c.before,c.after])if(value!==null&&(!value||typeof value!=='object'||Array.isArray(value)||value.id!==c.id))fail('Invalid undo value.');}const keys=new Set();for(const c of item.settings){if(!UNDO_SETTINGS.includes(c.key)||keys.has(c.key))fail('Invalid undo setting.');keys.add(c.key);}}
 }
 for(const [key,allowed] of [['language',['en','ar']],['textSize',['standard','large','larger']]])if(d.settings?.[key]!==undefined&&!allowed.includes(d.settings[key]))fail('Invalid display preference.');
 if(d.settings?.setupStep!==undefined&&(!Number.isInteger(d.settings.setupStep)||d.settings.setupStep<0||d.settings.setupStep>4))fail('Invalid setup step.');
 for(const key of ['setupDone','backupReminders'])if(d.settings?.[key]!==undefined&&typeof d.settings[key]!=='boolean')fail('Invalid preference.');
 if(d.settings?.backupReminderDays!==undefined&&![3,7,14,30].includes(d.settings.backupReminderDays))fail('Choose a valid backup interval.');
 if(d.settings?.backupReminderChanges!==undefined&&![10,20,50].includes(d.settings.backupReminderChanges))fail('Choose a valid change reminder.');
 if(d.settings?.backupSnoozeUntil!==undefined&&!Number.isFinite(d.settings.backupSnoozeUntil))fail('Invalid backup reminder date.');
 if(d.settings?.backupBaseline!==undefined&&(!Number.isFinite(d.settings.backupBaseline.at)||!Number.isSafeInteger(d.settings.backupBaseline.counter)||d.settings.backupBaseline.counter<0))fail('Invalid backup verification baseline.');
 ...
 const payment=d.settings?.defaultPayment;if(payment){const aid=payment.startsWith('card:')?d.cards.find(c=>'card:'+c.id===payment)?.account:payment;if(d.accounts.some(a=>a.id===aid&&a.archived))fail('Choose an active default payment account.');}if(payment!==undefined&&(typeof payment!=='string'||payment&&!accounts.has(payment)&&!d.cards.some(c=>'card:'+c.id===payment)))fail('Choose an existing default payment account.');
...
return {VERSION,MAX_MONEY,activeAccounts,accountRemoval,possibleDuplicates,duplicateToken,recurringMatches,backupReminder,DEFAULT_QUICK,FREQUENCIES,quickCategories,nextOccurrence,dueRecurring,inTrackingPeriod,ACCOUNT_TYPES,COLORS,CATEGORIES,GOAL_ICONS,uid,clone,money,format,localDate,validDate,blank,canSetOpening,resetMoney,shareQuantity,holdingsValue,balances,reserves,goalSaved,summary,spendingBreakdown,goalPlan,apply,validate,calc,assistant,guessCategory,demo};
});
