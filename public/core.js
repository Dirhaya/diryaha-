...
function resetMoney(state){const fresh=blank();fresh.rev=state.rev+1;fresh.accounts=state.accounts.map(a=>{const account={...clone(a),opening:0};delete account.lastCheck;delete account.holdings;return account});fresh.cards=clone(state.cards);fresh.goals=state.goals.map(g=>({...clone(g),allocations:[]}));if(state.settings.reminderTimes)fresh.settings.reminderTimes=clone(state.settings.reminderTimes);for(const key of ['name','theme','hideAmounts','needsName','gharsWelcomed'])fresh.settings[key]=state.settings[key];validate(fresh);return fresh}
