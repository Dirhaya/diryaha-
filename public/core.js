...
function resetMoney(state){const fresh=blank();fresh.rev=state.rev+1;fresh.accounts=state.accounts.map(a=>{const account={...clone(a),opening:0};delete account.lastCheck;delete account.holdings;return account});fresh.cards=clone(state.cards);fresh.goals=state.goals.map(g=>({...clone(g),allocations:[]}));if(state.settings.reminderTimes)fresh.settings.reminderTimes=clone(state.settings.reminderTimes);for(const key of ['name','theme','hideAmounts'])fresh.settings[key]=state.settings[key];validate(fresh);return fresh}
...
function apply(state,action){
 ...
 switch(action.type){
  ...
  case 'settings':{if(action.reminderTimes!==undefined){if(!Array.isArray(action.reminderTimes)||action.reminderTimes.length>24||action.reminderTimes.some(t=>typeof t!=='string'||!/^([01]\d|2[0-3]):[0-5]\d$/.test(t)))fail('Choose up to 24 valid reminder times.');s.settings.reminderTimes=[...new Set(action.reminderTimes)].sort();}if(action.name!==undefined){s.settings.name=cleanName(action.name,'Your name',35);s.settings.needsName=false;}if(action.theme!==undefined){if(!['beige','grey'].includes(action.theme))fail('Choose beige or grey.');s.settings.theme=action.theme}if(action.hideAmounts!==undefined)s.settings.hideAmounts=!!action.hideAmounts;if(action.monthlyBudget!==undefined)s.settings.monthlyBudget=cents(action.monthlyBudget);if(action.lastBackup!==undefined)s.settings.lastBackup=action.lastBackup;break}
...
function assistant(state,input){
 ...
 try{
  ...
  if(period){const now=new Date(),previous=localDate(new Date(now.getFullYear(),now.getMonth()-1,1)),last=summary(state,previous),value=period[1]==='spending'?last.spent:period[1]==='salary'?last.salary:last.income;return {text:`Recorded ${period[1]} for ${last.month}: AED ${format(value)}. Transfers and opening balances are excluded.`}}
  const categoryQuery=q.match(/^spending on (.+?)(?: (this|last) month)?[?]?$/);
  if(categoryQuery){const alias={food:['Food & drink','Groceries'],'food & drink':['Food & drink','Groceries'],groceries:['Groceries'],health:['Health'],gifts:['Gifts']},name=categoryQuery[1].trim(),categories=alias[name]||CATEGORIES.filter(c=>c.toLowerCase()===name&&!INCOME_CATEGORIES.includes(c));if(!categories.length)return {text:'Choose a spending category, for example: spending on food this month, or spending on transport last month.'};const now=new Date(),date=categoryQuery[2]==='last'?localDate(new Date(now.getFullYear(),now.getMonth()-1,1)):localDate(),totals=summary(state,date),value=safeSum(categories.map(c=>totals.categories[c]||0));return {text:`${categories.join(' + ')} in ${totals.month}: AED ${format(value)}. Based on your recorded categories; refunds are not subtracted.`}}
...
return {VERSION,MAX_MONEY,ACCOUNT_TYPES,COLORS,CATEGORIES,GOAL_ICONS,uid,clone,money,format,localDate,validDate,blank,canSetOpening,resetMoney,shareQuantity,holdingsValue,balances,reserves,goalSaved,summary,spendingBreakdown,goalPlan,apply,validate,calc,assistant,guessCategory,demo};
});
