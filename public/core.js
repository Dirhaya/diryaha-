...
function apply(state,action){
 ...
 switch(action.type){
  ...
  case 'account.edit':{const a=getAccount(action.id),name=cleanName(action.name,'Account name');uniqueName(s.accounts,name,a.id,'account');Object.assign(a,{name,type:ACCOUNT_TYPES.includes(action.kind)?action.kind:a.type,color:COLORS.includes(action.color)?action.color:a.color});break}
...
return {VERSION,MAX_MONEY,ACCOUNT_TYPES,COLORS,CATEGORIES,GOAL_ICONS,uid,clone,money,format,localDate,validDate,blank,balances,reserves,goalSaved,summary,goalPlan,apply,validate,calc,assistant,guessCategory,demo};
});
