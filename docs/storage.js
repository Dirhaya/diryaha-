...
function commit(transform,{backup=false}={}){return new Promise((resolve,reject)=>{const tx=db.transaction('vault','readwrite');const store=tx.objectStore('vault');const req=store.get('main');let next,error;req.onsuccess=()=>{try{const current=req.result?.data||Core.blank();next=transform(current);Core.validate(next);if(backup)store.put({id:'before-restore',data:current});store.put({id:'main',data:next})}catch(e){error=e;tx.abort()}};tx.oncomplete=()=>{channel?.postMessage({rev:next.rev});resolve(next)};tx.onabort=tx.onerror=()=>reject(error||tx.error||new Error('The change could not be saved.'))})}
...
root.DirhayaStorage={init,get,mutate,restore,recover};
})(globalThis);
