...
function apply(state,action,{recordUndo=true}={}){
 ...
 switch(action.type){
  ...
  case 'account.delete':{if((s.favourites||[]).some(f=>f.account===action.id))fail('Remove favourites using this account first.');if((s.recurring||[]).some(r=>r.account===action.id))fail('Remove recurring entries using this account first.');if(s.settings.defaultPayment===action.id)s.settings.defaultPayment='';if(getAccount(action.id).holdings?.length)fail('Remove the holding details before deleting this account.');if(s.cards.some(c=>c.account===action.id)||s.transactions.some(t=>t.account===action.id||t.to===action.id)||s.goals.some(g=>g.allocations.some(a=>a.account===action.id)))fail('This account has transactions or goal savings. Keep it to preserve your history.');s.accounts=s.accounts.filter(a=>a.id!==action.id);break}
