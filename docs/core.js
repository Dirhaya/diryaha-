...
function resetMoney(state){const fresh=blank();fresh.rev=state.rev+1;fresh.accounts=state.accounts.map(a=>{const account={...clone(a),opening:0};delete account.lastCheck;delete account.holdings;return account});fresh.cards=clone(state.cards);fresh.goals=state.goals.map(g=>({...clone(g),allocations:[]}));for(const key of ['name','theme','hideAmounts'])fresh.settings[key]=state.settings[key];validate(fresh);return fresh}
